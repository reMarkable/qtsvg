...
if [ ! -d "$qtbase_dir" ] ; then
    mkdir -p "$qtbase_dir"
    git clone \
        https://github.com/remarkable/qtbase.git \
        --branch 6.8.2#wasi \
        --single-branch \
        --depth=1 \
        "$qtbase_dir"
...
cmake . \
    ...
    -DQt6BundledLibjpeg_DIR="$qtbase_build_dir/lib/cmake/Qt6BundledLibjpeg" \
...
pushd ./build/wasm32-wasi-install && tar -czvf ../qtsvg-wasm32-wasi-install.tar.gz . && popd
